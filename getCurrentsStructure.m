...
INaCa_fractionSS2 = 0.3; if (isfield(parameters, 'INaCa_fractionSS2')) INaCa_fractionSS2 = parameters.INaCa_fractionSS2; end
INa_fractionSS2 = 0.2; if (isfield(parameters, 'INa_fractionSS2')) INa_fractionSS2 = parameters.INa_fractionSS2; end
...
Vuni_Multiplier = 1; if (isfield(parameters,'Vuni_Multiplier')) Vuni_Multiplier = parameters.Vuni_Multiplier; end
VNaCa_Multiplier = 1; if (isfield(parameters,'VNaCa_Multiplier')) VNaCa_Multiplier = parameters.VNaCa_Multiplier; end
...
currents.camit = zeros(nPoints, 1);
currents.INaCamit = zeros(nPoints, 1);
currents.ICauni = zeros(nPoints, 1);
currents.Vhe = zeros(nPoints, 1);
currents.Vhu = zeros(nPoints, 1);
currents.Vant = zeros(nPoints, 1);
...
for iBeat = 1:size(time, 1)
    ...
    for j=1:size(X{iBeat},1)
       IsJs=modelNow(time{iBeat}(j),X{iBeat}(j,:),0, cellType, ICaL_Multiplier, ...
        ...
        currents.camit(i) = IsJs(37);
        currents.INaCamit(i) = IsJs(38);
        currents.ICauni(i)= IsJs(39);
        currents.Vhe(i) = IsJs(40);
        currents.Vhu(i) = IsJs(41);
        currents.Vant(i) = IsJs(42);
